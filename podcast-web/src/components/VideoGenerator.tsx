...
import { Video, Play, Check, AlertCircle, Loader } from "lucide-react";

interface VideoGeneratorProps {
  folder: string;
  podcast: PodcastId;
  hasComplete: boolean;
  hasYoutubeCover: boolean;
  hasVideo: boolean;
}
...
export function VideoGenerator({
  ...
  const canGenerate = hasComplete && hasYoutubeCover && !generating;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (json.success) {
        setResult({ success: true, message: "Video generado correctamente" });
      } else {
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
      ...
      <div className="space-y-2 text-sm text-gray-500 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>Cover YouTube (*Y.jpg/png)</span>
        </div>
        <div className="flex items-center gap-2">
          {hasVideo ? (
            <Check size={14} className="text-green-500" />
      ...
      {(!hasComplete || !hasYoutubeCover) && (
        <p className="mt-2 text-xs text-yellow-600 dark:text-yellow-400">
          Se necesita el audio completo y el cover YouTube
        </p>
      )}
